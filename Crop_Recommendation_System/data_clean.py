...
market_edited = market_price[market_price['commodity'].str.lower().isin(
    [x.lower() for x in products])]


grouped_market = market_edited.groupby(
    ['state', 'commodity']).agg(['mean']).reset_index()
grouped_market.columns = ["".join(x) for x in grouped_market.columns.ravel()]

grouped_market['modal_pricemean'] = grouped_market['modal_pricemean'].apply(
...
for i in range(0, yield_np_mat.shape[0]-1):
    for j in range(1, yield_np_mat.shape[1]):
        if(yield_np_mat[i][j] != 0.0):
            rs_per_kg = grouped_market[(grouped_market['state'] == states[i]) & (
                grouped_market['commodity'] == crops[j-1])]['modal_pricemean']
            if(rs_per_kg.shape[0] == 1):
                diff = ((yield_np_mat[i][j]) * rs_per_kg.values[0]
                        ) - coc_dict[str(crops[j-1].lower())]
                val = diff if diff > 0.0 else 1.0
            else:
                diff = ((yield_np_mat[i][j]) * std_prices[str(crops[j-1].lower())]
                        ) - coc_dict[str(crops[j-1].lower())]
                val = diff if diff > 0.0 else 1.0
